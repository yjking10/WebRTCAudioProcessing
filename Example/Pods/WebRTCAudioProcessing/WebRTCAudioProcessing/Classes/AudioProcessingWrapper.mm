...
#pragma mark - Audio Processing (Float32)QWEN
...
#pragma mark - Audio Processing (Float32)GPT

- (NSData *)processAudioFrameFloat2:(NSData *)pcmData
                        sampleRate:(int)sampleRate
                          channels:(int)channels {
    if (!pcmData || pcmData.length == 0) return nil;
    if (sampleRate <= 0 || channels <= 0) return nil;

    NSUInteger frameCount = pcmData.length / (sizeof(float) * channels);
    const float *interleavedInput = (const float *)pcmData.bytes;

    // 准备分离声道的 buffer
    NSMutableArray<NSMutableData *> *channelData = [NSMutableArray arrayWithCapacity:channels];
    float *inputChannels[channels];
    for (int ch = 0; ch < channels; ch++) {
        NSMutableData *buf = [NSMutableData dataWithLength:frameCount * sizeof(float)];
        [channelData addObject:buf];
        inputChannels[ch] = (float *)buf.mutableBytes;
    }

    // 拆分 interleaved → planar
    for (NSUInteger i = 0; i < frameCount; i++) {
        for (int ch = 0; ch < channels; ch++) {
            inputChannels[ch][i] = interleavedInput[i * channels + ch];
        }
    }

    // 输出声道 buffer
    NSMutableArray<NSMutableData *> *outChannelData = [NSMutableArray arrayWithCapacity:channels];
    float *outputChannels[channels];
    for (int ch = 0; ch < channels; ch++) {
        NSMutableData *buf = [NSMutableData dataWithLength:frameCount * sizeof(float)];
        [outChannelData addObject:buf];
        outputChannels[ch] = (float *)buf.mutableBytes;
    }

    webrtc::StreamConfig config(sampleRate, channels);

    int result = _apm->ProcessStream((const float * const *)inputChannels,
                                     config,
                                     config,
                                     outputChannels);
    ...
    // 再次打包成 interleaved
    NSMutableData *processedData = [NSMutableData dataWithLength:pcmData.length];
    float *interleavedOutput = (float *)processedData.mutableBytes;

    for (NSUInteger i = 0; i < frameCount; i++) {
        for (int ch = 0; ch < channels; ch++) {
            interleavedOutput[i * channels + ch] = outputChannels[ch][i];
        }
    }
